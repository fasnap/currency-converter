...
def load_cache():
    ...
    if os.path.exists(CACHE_FILE):
        with open(CACHE_FILE, 'r') as file:
            ...
            if cached_data['last_fetch'] is not None:
                cached_data['last_fetch']=datetime.fromisoformat(cached_data['last_fetch'])
            return cached_data
...
def convert_currency(amount,from_currency,to_currency):
    ...
    amount_in_usd = amount/rates[from_currency] if from_currency!='USD' else amount
...
# Function to save conversion to a local database
def save_conversion_to_db(amount, from_currency, to_currency, converted_amount, exchange_rate):
    cursor.execute('''INSERT INTO conversions (amount,from_currency,to_currency, 
        converted_amount,exchange_rate,date)
        VALUES(?, ?, ?, ?, ?, ?)''',
        (amount, from_currency, to_currency, converted_amount, exchange_rate,datetime.now().date()))
    conn.commit()
...
def main():
    ...
    if converted_amount is not None:
        save_conversion_to_db(amount, from_currency, to_currency, converted_amount, exchange_rate)
    ...
    show_history=input("Would you like to see your conversion history? (yes/no):").lower()
    if show_history == 'yes':
